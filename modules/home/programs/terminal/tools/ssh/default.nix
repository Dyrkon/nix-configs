{
  ...
  default-key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAbNMaXMPTmpywYHDOLILlo8trCK49Up2Sz4DsKfNJlF";
  ...
  other-hosts-config = lib.concatMapStringsSep "\n" (
    name: let
      ...
      forward-gpg =
        ...
        '';
      port-expr =
        if builtins.hasAttr name inputs.self.nixosConfigurations
        then "Port ${builtins.toString cfg.port}"
        else "";
    in ''
      Host ${name}
        Hostname ${name}.local
